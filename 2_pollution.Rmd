...
madrid_pollution <- madrid %>% 
  filter(EBE !=(is.na = TRUE)) %>% 
  select(year, EBE, mnth) %>% 
  group_by(year, mnth) %>% 
  summarise(max_EBE_month = max(EBE))
...
madrid_plot <- madrid_pollution %>% 
  ggplot(aes(x = mnth, y = max_EBE_month)) +
  geom_line() +
  geom_point() + 
  facet_wrap(~year) +
  labs(x = "Month", y = "Maxmimum Ethylbenzene (μg/m³)") +
...
```{r subtract-dfs}
pollutant_difference <- pollution_2001 - pollution_2006
...
```{r pollution_diff}
pollution_diff <- pollutant_difference %>% 
  pivot_longer(cols = everything(), names_to = "pollutant", values_to = "difference")

print(pollution_diff)
```
